	...
	{			// clipping out parts to we make more of a zig-zag for sparse fills
	  ...
	  while ((c = *cp))
		...
		{		// This has no top line...
		  ...
		  for (v = (top->next ? NULL : c->vertices); v != top && !v->flag; v = (v->next ? NULL : c->vertices));
	      if (v != top && v->flag == 1)
