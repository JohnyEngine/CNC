  ...
  while (*cc)
    {
      ...
      while (a)
	{
	  poly_vertex_t *b = (a->next ? NULL : contour->vertices);
	  poly_vertex_t *c = (b->next ? NULL : contour->vertices);
#ifdef	POLY_FLOAT
...
void split_line(segment_t * p, poly_dim_t x, poly_dim_t y, int *splits, segment_t **queue)
{
	if (x == p->ax && y == p->ay)
		return;
	if (x == p->bx && y == p->by)
...
void intersect_check(segment_t * a, segment_t * b, int *splits, segment_t **queue)
...
void segment_add(segment_t * q, segment_t **sweep, poly_dim_t *lastx, int *segcount, segment_t **stage2, int *splits, segment_t **queue)
{				// add segment, in order
	...
	for (s = *sweep; s; s = s->next)
		intersect_check(s, q, splits, queue);
	q->next = sweep;
	sweep = q;
	if (q->ax != *lastx)
		segment_tidy((*lastx) = q->ax, sweep, segcount, stage2);
}
...
void paths_close(poly_dim_t x, polygon_t *new, point_t **points, path_t **paths)
{
	point_t *p, **pp = points;
	while ((p = *pp))
	{
...
{				// return set of simple contours from one or more input polygons
	...
	for (p = q->contours; p; p = p->next)
	  {
	    for (a = p->vertices; a; a = a->next)
	      {
		poly_vertex_t *b = (a->next ? NULL : p->vertices);
  ...
  while (1)
	...
	{			// Sweep
	  ...
	  segment_add (s, &sweep, &lastx, &segcount, &stage2, &splits, &queue);
	}
      segment_tidy (POLY_DIM_MAX, &sweep, &segcount, &stage2);
      if (!splits)
	break;
  ...
  while (stage2)
	...
	{			// start new column
	  ...
	  paths_close (lastx = s->ax, new, &points, &paths);
	  yp = &points;
	  wind = 0;
	}
      while ((p = *yp))
  ...
  paths_close (POLY_DIM_MAX, new, &points, &paths);
  if (paths)
...
void show(const char *name, polygon_t *i)
{
...
#define test(x) i=poly_new();{printf("Test %s\n",#x);int a;for(a=0;a<sizeof(x)/sizeof(*x);a++){poly_start(i);int*p=x[a];int f=((*p<0)?1:0);int b=abs(*p++);while(b--){poly_add(i,p[0],p[1],f);p+=2;}};show(#x, i);}
  int *overlap2[] = {
    (int[]) {
